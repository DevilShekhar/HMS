...
Auth::routes();
    ...
    Route::prefix('{restaurant}')->middleware('restaurant')->group(function () {
        Route::get('/', function () {$restaurant = app('restaurant');
            if (!auth()->check()) {
        ...
        Route::middleware(['auth'])->group(function () {
            Route::get('/dashboard', function () {$restaurant = app('restaurant');
                if (auth()->user()->role == 'super_admin') {abort(403);
            }
            return view('admin.restaurants.dashboard',compact('restaurant'));})->name('restaurant.dashboard');
