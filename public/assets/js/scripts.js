...
document.querySelectorAll('.plan-radio').forEach(radio => {
    radio.addEventListener('change', function () {
        ...
        document.querySelectorAll('.duration-select').forEach(select => {
            ...
            if (select.dataset.planId == planId) {

                select.style.display = 'block';
                select.setAttribute('name', 'billing_cycle');
            }
        });
    });
});
document.querySelectorAll('.duration-select').forEach(select => {
    select.addEventListener('change', function () {
        ...
        card.querySelector('.plan-price')
            .textContent = '₹' + parseFloat(price).toLocaleString();
    });
});
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (checkedRadio) {
        ...
        const selectedPlanId = checkedRadio.value;

        const select = document.querySelector(
            `.duration-select[data-plan-id="${selectedPlanId}"]`
        );
        ...
        if (select) {
            ...
            card.querySelector('.plan-cycle')
                .textContent = option.text.split('-')[0].trim();
        }
    }
